...
public interface JpaProductsRepository extends JpaRepository<Product,Long> {
    ...
    @Query(value = "select  BIN_TO_UUID(p.product_id) as ProductId, p.product_name as ProductName,p.price as Price,p.product_description as ProductDescription,pI.image_path as ImagePath,p.shop_shop_id as ShopId from product_images as pI join products as p on p.product_id = pI.product where p.product_category_category_id in ?2 and pI.image_path like '%1' and p.deleted = ?1",
    ...
    @Query(value = "select  BIN_TO_UUID(p.product_id) as ProductId, p.product_name as ProductName,p.price as Price,p.product_description as ProductDescription,pI.image_path as ImagePath,p.shop_shop_id as ShopId from product_images as pI join products as p on p.product_id = pI.product where pI.image_path like '%1' and p.deleted = ?1 ",
    ...
    @Query(value = "select  BIN_TO_UUID(p.product_id) as ProductId, p.product_name as ProductName,p.price as Price,p.product_description as ProductDescription,pI.image_path as ImagePath,p.shop_shop_id as ShopId from product_images as pI join products as p on p.product_id = pI.product where p.product_category_category_id in ?2  and p.shop_shop_id=?3 and pI.image_path like '%1' and p.deleted = ?1",
            countQuery = "select count(*) from products  join product_images on products.product_id = product_images.product where products.product_category_category_id in ?2 and products.shop_shop_id=?3 and product_images.image_path like '%1' and products.deleted = ?1 ",
            nativeQuery = true)
    ...
    @Query(value = "select  BIN_TO_UUID(p.product_id) as ProductId, p.product_name as ProductName,p.price as Price,p.product_description as ProductDescription,pI.image_path as ImagePath,p.shop_shop_id as ShopId from product_images as pI join products as p on p.product_id = pI.product where p.shop_shop_id=?2 and pI.image_path like '%1' and p.deleted = ?1 ",
            countQuery = "select count(*) from products join product_images on products.product_id = product_images.product where products.shop_shop_id=?2 and  product_images.image_path like '%1' and products.deleted = ?1 ",
            nativeQuery = true)
