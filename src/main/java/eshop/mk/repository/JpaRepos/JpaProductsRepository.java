...
public interface JpaProductsRepository extends JpaRepository<Product,Long> {
    ...
    @Query("select new eshop.mk.model.modelDTOS.ProductDTO(p.productId,p.productName,p.productDescription,p.price,pI.imagePath) from Product p join p.productImages as pI on p.productId=pI.product where p.productId=:productId and p.deleted=false")
    ...
    @Query(value = "select  BIN_TO_UUID(p.product_id) as ProductId, p.product_name as ProductName,p.price as Price,p.product_description as ProductDescription,pI.image_path as ImagePath from product_images as pI join products as p on p.product_id = pI.product where p.shop_shop_id=?2 and pI.image_path like '%1' and p.deleted = ?1 ",
